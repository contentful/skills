        ...
        ${render.kv({
          ...
          Architecture:
            store.setup?.architecture === 'client-only'
              ? 'Client-only — evaluated in the browser; renders baseline content first, then swaps to the variant (a visible flash, or a hidden slot until it resolves). Keeps the page HTML static/CDN-cacheable.'
              : store.setup?.architecture === 'hybrid-ssr'
                ? 'Hybrid SSR — server preflight resolves personalization before markup is sent (no baseline flash), then the browser hydrates and takes over. The personalized routes render per request, so they are not shared-CDN-cacheable; other routes and raw baseline data still cache.'
