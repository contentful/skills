...
test("detects unpublished experience entries (unresolved links)", async () => {
  ...
  try {
    ...
    const finding = result.findings.find((f) =>
      f.item.includes("Experience entries"),
    );
    assert.ok(finding);
    assert.equal(finding.status, "fail");
...
test("detects unpublished variant entries", async () => {
  const entry = makeEntry({
    title: "Test",
    nt_experiences: [
      makeExperience("exp1", [unresolvedLink("var1"), unresolvedLink("var2")]),
    ],
  });
  const originalFetch = globalThis.fetch;
  ...
  try {
    ...
    assert.equal(result.status, "fail");
    const finding = result.findings.find((f) =>
      f.item.includes("Variant entries"),
    );
    assert.ok(finding);
    assert.equal(finding.status, "fail");
    assert.ok(finding.detail.includes("not published"));
...
test("handles invalid CDA token (401)", async () => {
  ...
  try {
    ...
    assert.equal(result.status, "fail");
    const finding = result.findings.find((f) =>
      f.item.includes("authentication"),
    );
    assert.ok(finding);
    assert.equal(finding.status, "fail");
  } finally {
...
test("warns when nt_experiences exists but is empty", async () => {
  ...
  try {
    ...
    const finding = result.findings.find((f) =>
      f.item.includes("Experience attachment"),
    );
    assert.ok(finding);
