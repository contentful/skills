  ...
  const result = await runComposite(skill, {
    model: mockModel({
      ...
      "doctor/check-api": {
        status: "skip",
        findings: [
          { item: "Ninetailed API", status: "skip", detail: "No credentials" },
        ],
        reachable: false,
      },
      "doctor/triage": {
  ...
  const result = await runSkill(doctorSkill, {
    model: mockModel({
      explore: {
        ...
        projectPath: ".",
        explorationSummary:
          "Next.js 14 App Router project with partial Ninetailed setup",
        ...
        recommendations: [
          {
            priority: "critical",
            message:
              "Entry has unpublished changes — republish the baseline entry",
            category: "content",
          },
        ],
